    ...
    switch (type) {
        ...
        case SELECT_OPERATION:
            ...
            if (!payload.isCaretaker) {
                myOperation = state.caretakerOperations.find((op) => op.operation.id === payload.id);
            } else {
                myOperation = state.operations.find((op) => op.operation.id === payload.id);
            }
        ...
        case FILTER_BY_DATE:
            ...
            const filterByDate = state.operations.filter((op) => {
                const time = differenceInCalendarDays(new Date(), new Date(op.operation.createdAt));
                if (time <= payload) {
                    return op;
                }
            });
