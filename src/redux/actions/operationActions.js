...
export const updateOperationStatus = (token, uid, operationId) => async (dispatch) => {
    try {
...
export const captureOperation = (token, PayerID) => async (dispatch) => {
    try {
        const { data } = await api.get(`/operations/capture-order?token=${token}&PayerID=${PayerID}`);
...
export const cancelOperation = (token) => async (dispatch) => {
    try {
        const { data } = await api.get(`/operations/cancel-order?token=${token}`);
